...
# low plot
axs[0].scatter(P_global, f_mcm(mu_est=mu, b=4, P=P_global), alpha=1)
# axs[0].scatter(P_global, f_mcm(mu_est=mu, b=4, P=P_global), alpha=1, marker='+', color='black')

# mid plot
axs[1].scatter(P_global, f_mcm(mu_est=mu, b=6, P=P_global), alpha=1)
# axs[1].scatter(P_global, f_mcm(mu_est=mu, b=6, P=P_global), alpha=1, marker='+', color='black')

# upper plot
axs[2].scatter(P_global, f_mcm(mu_est=mu, b=8, P=P_global), alpha=1)
# axs[2].scatter(P_global, f_mcm(mu_est=mu, b=8, P=P_global), alpha=1, marker='+', color='black')


# axs[2].legend(labels=['µ', 'µ+0.1'], title='', loc='lower right')
axs[0].set_ylabel('         ')  # add whitespace on the left
axs[0].set_title('b = 4', fontsize='12')
axs[1].set_title('b = 6', fontsize='12')
axs[2].set_title('b = 8', fontsize='12')
for i in range(3):
